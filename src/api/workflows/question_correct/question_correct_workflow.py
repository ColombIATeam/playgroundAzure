...
class QuestionCorrectWorkflow:
    ...
    def __init__(
        ...
        self._gpt_question_correct = GptQuestionCorrect()
        self.eval_switch = eval_switch

    async def execute(self, request:ListQuestionCorrectRequest) -> ListQuestionCorrectResponse:
        logging.info(f"Executing QuestionCorrectWorkflow request={request}")
        correct_answers = await self._gpt_question_correct.get_question_correct(input_prompt=request.prompt_table,
                                                                          questions=str(request.Questions))
        if self.eval_switch:
            prompt_improving = await self._gpt_question_correct.get_question_correct_eval(prompt_improving=request.prompt_table)
