...
class TestWorkflow:
    ...
    def __init__(self,
        ...
        self._gpt_test = GptTest()
        self.eval_switch = eval_switch
        
    async def execute(self, request: TestRequest) -> GenerarTestsResponse:  
        preguntas = await self._gpt_test.get_text_test(request.text, request.prompt)  
        if self.eval_switch:  
            prompt_improving = await self._gpt_test.get_question_eval(prompt_improving=request.prompt)  
